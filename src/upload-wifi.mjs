...
class WiFiUploader {
    ...
    sendFileData_V1(pageIndex) {
        ...
        packet[4] = 238; // 0xEE
        packet[5] = 255; // 0xFF
        // Pairing code exactly as in successful exchange: 07 ee fd
        packet[6] = (this.pairingCode >> 16) & 0xFF; // 0x07
        packet[7] = (this.pairingCode >> 8) & 0xFF;  // 0xEE  
        packet[8] = this.pairingCode & 0xFF;         // 0xFD
